...
# def pos_enc_rope_2(xs, dim, alpha=1000):
#     """Positional encoding.
...
#     #omega=alpha * torch.logspace(0, 2 / (dim) - 1, (dim), 100, **kwargs)
#     omega= alpha * torch.logspace(0, 2 / (dim) - 1, (dim), 100)
#     p1 = -(xs * omega).sin()
#     p2 = (xs * omega).cos()
#     return (p1, p2)
...
class PositionEncoder(nn.Module):
    ...
    def forward(self, inputs: dict):
        ...
        for field in self.fields:
            pos_enc_fn = pos_enc_symmetric if field in self.sym_fields else pos_enc  #rope_enc #
...
class FourierPositionEncoder(nn.Module):
    ...
    def __init__(self, input_name: str, dim: int, fields: list[str], scale: float = 1) -> None:
        ...
        ###adjust to try and make more comparable
        ...
        scales = torch.tensor([r_scale if field == "r" else default_scale for field in self.fields])
        
        # Register it as a buffer
        self.register_buffer('scale_tensor', scales)

        ##
        self.B = torch.nn.parameter.Buffer(torch.randn((len(fields), dim // 2)))
...
class RopeEncoder(nn.Module):
    def __init__(self, input_name: str, fields: list[str], sym_fields: list[str] | None = None, alpha=1000):#, dim: int
        """Positional encoder.
...
# class RopeEncoder_sep(nn.Module):#rotate on different parts
#     def __init__(self, input_name: str, fields: list[str], sym_fields: list[str] | None = None, alpha=1000):#, dim: int
#         """Positional encoder.
...
#     def onefield(self, inputs: dict, x):
#         """Apply positional encoding to the inputs.
...
#         lenin=len(x[0])
#         # Determine the target device from an input tensor
#         target_device = next(iter(inputs.values())).device
...
#     def forward(self, inputs: dict, x):
#         for field in self.fields: